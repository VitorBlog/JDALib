...
open class HttpRequest(
	...
	fun addHeader(key: String, value: Any?): HttpRequest {
		headers[key] = value.toString(); return this
	}

	fun setContent(value: Any?): HttpRequest {
		content = value.toString(); return this
	}

	fun join(): HttpRequest {
		join = true; return this
	}
	...
	fun get(success: Consumer<String>?, error: Consumer<Exception>?) {
		method = Method.GET; send(success, error)
	}

	fun post(success: Consumer<String>?, error: Consumer<Exception>?) {
		method = Method.POST; send(success, error)
	}
